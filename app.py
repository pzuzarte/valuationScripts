...
SCRIPTS = {
    ...
    "Earnings Quality": {
        "path": os.path.join(ROOT, "25_earningsQuality", "earningsQuality.py"),
        "desc": "Earnings Quality & Accruals Screener -- ranks US large caps by how well reported profits are backed by real cash. Uses the Sloan accruals ratio (NI - operating cash flow)/assets, cash conversion (CFO/NI), and FCF conversion. Flags companies whose earnings run ahead of cash (a documented underperformance signal).",
        "icon": "🔬",
        "params": [
            dict(id="limit", label="Universe size", type="option", flag="--limit",
                 required=False, default="500", values=["120", "300", "500"],
                 hint="Number of top US large caps to screen by market cap"),
            dict(id="min_mktcap", label="Min market cap ($)", type="entry",
                 flag="--min-mktcap", required=False, default="2e9",
                 hint="Minimum market cap filter (e.g. 2e9 = $2B)"),
        ],
    },
    "Estimate Store": {
        "path": os.path.join(ROOT, "26_estimateStore", "estimateStore.py"),
        "desc": "Point-in-Time Analyst Estimate Store -- captures timestamped analyst estimates (forward EPS/revenue, price targets, ratings) to a local database so backtests can look up what was known AS OF a past date, removing look-ahead bias. Run monthly to accumulate history.",
        "icon": "🗄️",
        "params": [
            dict(id="report", label="Report only (skip capture)", type="check",
                 flag="--report", required=False, default=False,
                 hint="Build the coverage/revision report without capturing a new snapshot"),
            dict(id="query", label="Query a ticker (optional)", type="entry",
                 flag="--query", required=False, default="",
                 hint="Print stored estimate history for one ticker, e.g. NVDA"),
            dict(id="limit", label="Universe size", type="option", flag="--limit",
                 required=False, default="500", values=["300", "500", "800"]),
        ],
    },
    "Options Risk": {
        "path": os.path.join(ROOT, "27_optionsRisk", "optionsRisk.py"),
        "desc": "Options-Implied Risk Dashboard -- forward-looking risk from live option chains: ATM implied volatility, put/call skew (crash fear), term-structure slope (near-term stress), put/call open-interest ratio, and variance risk premium (IV vs realized vol).",
        "icon": "🎲",
        "params": [
            dict(id="tickers", label="Tickers (comma-separated)", type="entry",
                 flag="--tickers", required=True, default="NVDA,AAPL,SPY",
                 hint="e.g. NVDA,AAPL,TSLA,SPY"),
        ],
    },
    "Portfolio Optimizer": {
        "path": os.path.join(ROOT, "28_portfolioOptimizer", "portfolioOptimizer.py"),
        "desc": "Portfolio Optimizer -- long-only mean-variance optimization over a candidate set. Builds the efficient frontier, finds the max-Sharpe (tangency) and min-variance portfolios, shows the correlation heatmap, and reports VaR / drawdown for the recommended mix.",
        "icon": "⚖️",
        "params": [
            dict(id="tickers", label="Tickers (comma-separated)", type="entry",
                 flag="--tickers", required=True, default="NVDA,MSFT,AAPL,GOOGL,KO,JNJ",
                 hint="At least 2 tickers, e.g. NVDA,MSFT,AAPL,GOOGL,KO,JNJ"),
            dict(id="years", label="Years of history", type="option", flag="--years",
                 required=False, default="3", values=["2", "3", "5"]),
        ],
    },
}
